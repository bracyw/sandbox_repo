...
def test_index_pointers_are_replaced_on_overwrite():
    client = FakeS3Client()
    db = S3Database(bucket="cheap-db", client=client)

    db.put("users", "u1", {"plan": "free"}, index_fields=["plan"])
    db.put("users", "u1", {"plan": "pro"}, index_fields=["plan"])

    free = db.query_by_index("users", "plan", "free")
    pro = db.query_by_index("users", "plan", "pro")

    assert [r.id for r in free] == []
    assert [r.id for r in pro] == ["u1"]


def test_delete_removes_index_pointers():
    client = FakeS3Client()
    db = S3Database(bucket="cheap-db", client=client)
